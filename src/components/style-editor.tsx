...
export type StyleCategory = 'layout' | 'typography' | 'colors' | 'effects' | 'all';
...
export function StyleEditor() {
  ...
  const STYLE_TABS = [
    ...
    { id: 'effects', label: 'ขอบ & เงา', icon: Sun, count: 'ขอบ, เงา, แสง' },
    { id: 'all', label: 'ทั้งหมด', icon: Sliders, count: 'ดูทั้งหมด' },
  ] as const;

  return (
    <div className="flex-1 min-h-0 h-full flex flex-col overflow-hidden text-zinc-100">
      {/* 📌 Pinned Header: Sub-tabs + Reset Button (Locked at top, NEVER scrolls, ZERO overlap glitch) */}
      <div className="shrink-0 px-3 py-2.5 sm:px-4 bg-[#10101a] border-b border-zinc-800/90 shadow-md">
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="text-xs text-zinc-300 hover:text-orange-400 bg-zinc-900 hover:bg-zinc-800 border border-zinc-700/80 px-2.5 py-1.5 rounded-xl flex items-center gap-1.5 transition-all cursor-pointer shadow-sm shrink-0 font-medium whitespace-nowrap"
            title="รีเซ็ตค่ารูปแบบกลับเป็นค่าเริ่มต้น"
          >
            <RotateCcw className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">คืนค่าเริ่มต้น</span>
            <span className="sm:hidden">คืนค่า</span>
          </button>
        </div>
      </div>

      {/* 📜 Scrollable Body: Content Cards scroll underneath cleanly */}
      <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-5 space-y-4 scrollbar-thin scrollbar-thumb-zinc-800">
        {/* 🧭 Category 1: Placement, Layout & Spacing */}
        {(activeCategory === 'layout' || activeCategory === 'all') && (
          <>
            {/* 🧭 Tool Card 1: Placement, Alignment & Width (Placed 1st as requested by user) */}
            ...
            {/* 📐 Tool Card 2: Letter & Line Spacing */}
            <div className="p-4 sm:p-5 rounded-3xl bg-[#181824] border border-zinc-700/90 shadow-xl space-y-4 transition-all duration-200 group/card hover:bg-[#20202e] hover:border-orange-500/60 focus-within:bg-[#20202e] focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-500/30">
              <div className="flex items-center gap-2 pb-2.5 border-b border-zinc-700/70">
        ...
        {/* 🎨 Category 3: Text Color & Word Highlight & Background Box */}
        {(activeCategory === 'colors' || activeCategory === 'all') && (
          <>
            {/* 🎨 Tool Card 5: Text Color & Word Highlight */}
            <div className="p-4 sm:p-5 rounded-3xl bg-[#181824] border border-zinc-700/90 shadow-xl space-y-4 transition-all duration-200 group/card hover:bg-[#20202e] hover:border-orange-500/60 focus-within:bg-[#20202e] focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-500/30">
              <div className="flex items-center gap-2 pb-2.5 border-b border-zinc-700/70">
                ...
                {style.enableWordHighlight && (
                  <div className="space-y-4 pt-3 border-t border-zinc-700/80">
                    ...
                    <div className="space-y-2 pt-3 border-t border-zinc-800/80">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-zinc-200 font-medium flex items-center gap-1.5">
                          <span>แอนิเมชันและการแสดงคำ (Word Style & Animation):</span>
                        </span>
            ...
            {/* 📦 Tool Card 6: Background Box & Opacity (Grouped under colors) */}
            <div className="p-4 sm:p-5 rounded-3xl bg-[#181824] border border-zinc-700/90 shadow-xl space-y-4 transition-all duration-200 group/card hover:bg-[#20202e] hover:border-orange-500/60 focus-within:bg-[#20202e] focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-500/30">
              <div className="flex items-center gap-2 pb-2.5 border-b border-zinc-700/70">
        ...
        {/* ✨ Category 4: Shadow, Outline & Neon Glow Effects */}
        {(activeCategory === 'effects' || activeCategory === 'all') && (
          <div className="p-4 sm:p-5 rounded-3xl bg-[#181824] border border-zinc-700/90 shadow-xl space-y-4 transition-all duration-200 group/card hover:bg-[#20202e] hover:border-orange-500/60 focus-within:bg-[#20202e] focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-500/30">
            <div className="flex items-center gap-2 pb-2.5 border-b border-zinc-700/70">
              ...
              <h4 className="text-sm font-bold text-white">
                เงา เส้นขอบ และแสงเรืองแสง (Shadow, Stroke & Glow):
              </h4>
            </div>
            ...
            <div className="p-3.5 sm:p-4 rounded-2xl bg-[#0f0f18] border border-zinc-700/80 space-y-3 hover:border-zinc-500/80 focus-within:border-orange-500/60 transition-all">
              <div className="flex items-center justify-between">
                <label className="text-sm font-bold text-zinc-100 flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={Boolean(style.hasGlow)}
                    onChange={(e) => setStyle({ hasGlow: e.target.checked })}
                    className="w-4 h-4 accent-orange-500 rounded cursor-pointer"
                  />
                  <span className="flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                    <span>เปิดใช้งานแสงเรืองแสง (Neon Glow)</span>
                  </span>
                </label>

                {style.hasGlow && (
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-zinc-300">สีแสง:</span>
                    <input
                      type="color"
                      value={style.glowColor || '#FF6B00'}
                      onChange={(e) => setStyle({ glowColor: e.target.value })}
                      aria-label="เลือกสีแสงเรืองแสง"
                      className="w-6 h-6 rounded-lg bg-transparent border border-zinc-600 cursor-pointer overflow-hidden"
                    />
                  </div>
                )}
              ...
              {style.hasGlow && (
                <div className="space-y-3 pt-1 border-t border-zinc-700/80 text-sm">
                  {/* Neon Swatches */}
                  <div className="space-y-1.5">
                    <span className="text-xs text-zinc-300 font-medium block">
                      สียอดนิยม (Neon Palette):
                    </span>
                    <div className="flex flex-wrap items-center gap-2">
                      {NEON_GLOW_PALETTE.map((c) => (
                        <button
                          key={c.hex}
                          type="button"
                          onClick={() => setStyle({ glowColor: c.hex })}
                          className={`w-7 h-7 rounded-xl border transition-all cursor-pointer flex items-center justify-center ${
                            (style.glowColor || '#FF6B00').toLowerCase() === c.hex.toLowerCase()
                              ? 'ring-2 ring-orange-500 scale-110 border-white shadow-md'
                              : 'border-zinc-600 hover:border-zinc-400 hover:scale-105'
                          }`}
                          style={{ backgroundColor: c.hex }}
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-zinc-300">ความฟุ้งของแสง (Glow Blur):</span>
                      <span className="font-mono text-orange-300 font-bold bg-orange-500/15 px-2 py-0.5 rounded border border-orange-500/40 text-xs">
                        {style.glowBlur ?? 12}px
                      </span>
                    </div>
                    <input
                      type="range"
                      min={2}
                      max={30}
                      value={style.glowBlur ?? 12}
                      aria-label="ความฟุ้งของแสงนีออน"
                      onChange={(e) => setStyle({ glowBlur: parseInt(e.target.value, 10) })}
                      className="w-full accent-orange-500 cursor-pointer"
                    />
                    ...
                    <div className="grid grid-cols-4 gap-1.5 pt-1">
                      {[
                        { label: 'อ่อนๆ', val: 6 },
                        { label: 'พอดี', val: 12 },
                        { label: 'นีออนจัด', val: 20 },
                        { label: 'ฟุ้งสะใจ', val: 30 },
                      ].map((p) => (
                        <button
                          key={p.val}
                          type="button"
                          onClick={() => setStyle({ glowBlur: p.val })}
                          className={`p-1.5 rounded-xl border text-center flex flex-col items-center justify-center min-h-[42px] transition-colors cursor-pointer ${
                            (style.glowBlur ?? 12) === p.val
                              ? 'bg-gradient-to-r from-orange-500 to-amber-500 text-zinc-950 font-bold border-transparent shadow-md ring-1 ring-orange-400/40'
                              : 'bg-[#242434] border-zinc-700 hover:bg-[#2e2e42] hover:border-orange-400 text-zinc-100 shadow-sm'
                          }`}
                        >
                          <span className="block text-xs font-semibold leading-tight">{p.label}</span>
                          <span className="block text-[10px] opacity-85 font-mono leading-tight">({p.val}px)</span>
                        </button>
                      ))}
                    </div>
                  </div>
